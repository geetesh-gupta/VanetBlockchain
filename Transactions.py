...
from Functions import to_hex, to_str
        ...
        while(True):
            ...
            if(receipt is not None):
                if(receipt['status'] == '0x1'):
                    ...
                    print('newly deployed contract at address {}'.format(
                        contractAddress))
...
def send_txn(senderId, receiverId, msg, contractAddress):
    ...
    # from smart contract
    function = 'performTransaction(uint256,uint256,uint256)'
    methodId = w3.sha3(text=function)[0:4].hex()
    # param1 = to_hex(str(senderId))
    # param2 = to_hex(str(receiverId))
    # param3 = to_hex(msg)
    ...
    # send the transacton to your node
    print('executing {} with value {},{}, {}'.format(
        function, senderId, receiverId, msg))
    requestObject, requestId = createJSONRPCRequestObject(
        'eth_sendRawTransaction', params, requestId)
    responseObject = postJSONRPCRequestObject(URL, requestObject)
    try:
